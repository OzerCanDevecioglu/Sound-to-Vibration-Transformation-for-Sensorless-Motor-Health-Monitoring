...
class Generator(nn.Module):
    def __init__(self, filter=8):
        ...
        self.classifier = nn.Sequential(
                SelfONN1dlayer(in_channels=1,out_channels=16,kernel_size=81,q=3,padding=0,stride=8),
                torch.nn.Tanh(),
                SelfONN1dlayer(in_channels=16,out_channels=16,kernel_size=41,q=3,padding=0,stride=4),
                torch.nn.Tanh(),
                SelfONN1dlayer(in_channels=16,out_channels=16,kernel_size=21,q=3,padding=0,stride=4),
                torch.nn.Tanh(),
                SelfONN1dlayer(in_channels=16,out_channels=16,kernel_size=9,q=3,padding=0,stride=2),
                torch.nn.Tanh(),
                SelfONN1dlayer(in_channels=16,out_channels=16,kernel_size=7,q=3,padding=0,stride=2),
                torch.nn.Tanh(),
                torch.nn.Flatten(),
