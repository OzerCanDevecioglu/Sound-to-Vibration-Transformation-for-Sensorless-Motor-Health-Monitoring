...
class ECGDataset(Dataset):
    def __init__(self, base_img_paths, style_img_paths, phase='train'):
        ...
        self.phase = phase
        path_data_ch1 = loadmat('trmats/clear_sig.mat')
        label = loadmat('trmats/noisy_sig.mat')
        classinf = loadmat('trmats/label_sig.mat')
        self.data_ch1=path_data_ch1["clear_sig"]

        
        self.label=label["noisy_sig"]
        self.classinf=classinf["label_sig"]


        self.data=np.zeros((1,int(len(self.data_ch1)),4096))
        self.data2=np.zeros((1,int(len(self.label)),4096))
        self.data3=np.zeros((1,int(len(self.classinf)),2))

        self.data[0,:,:]=self.data_ch1
        self.data2[0,:,:]=self.label
        self.data3[0,:,:]=self.classinf
        ...
        self.class_inf = torch.tensor(self.data3).float()
    ...
    def normalize(self,x):
        for idx,data in enumerate(x):
            ...
            data *= 2
            data -= 1
            data =(data- data.mean(dim=-1).unsqueeze(-1))
            x[idx] = data
        return x
...
class TECGDataset(Dataset):
    def __init__(self, base_img_paths, style_img_paths,class_img_paths, phase='train'):

        self.base_img_paths = base_img_paths
        self.style_img_paths = style_img_paths
        self.class_img_paths = class_img_paths
        ...
        classinf = loadmat(self.class_img_paths[0])
    ...
    def __getitem__(self,index): 
        base = self.labels[:,index,:]
        style = self.inputt[:,index,:]#.unsqueeze(0)
        claaas = self.class_inf[:,index,:]#.unsqueeze(0)

        base = self.normalize(base)
        style = self.normalize(style)
        return base,style,claaas
...
class TECGDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        self.base_img_paths = glob.glob(os.path.join(self.data_dir, 'clear_sig.mat'))
        self.style_img_paths = glob.glob(os.path.join(self.data_dir, 'noisy_sig.mat'))
        self.class_img_paths = glob.glob(os.path.join(self.data_dir, 'label_sig.mat'))
    ...
    def train_dataloader(self):
        ...
        random.shuffle(self.base_img_paths)
        random.shuffle(self.style_img_paths)
        random.shuffle(self.class_img_paths)

        random.seed(self.seed)
        self.train_dataset = TECGDataset(self.base_img_paths, self.style_img_paths,self.class_img_paths, self.phase)
